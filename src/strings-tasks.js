...
function countVowels(str) {
  ...
  return str.split('').reduce((acc, cur) => {
    return vowelsList.includes(cur) ? acc + 1 : acc;
  }, 0);
}
...
function isPalindrome(str) {
  ...
  const strNormalized = str.replaceAll(/\W/g, '').toLowerCase();
  const len = Math.floor(strNormalized.length / 2);
  return (
    strNormalized.slice(0, len) ===
    strNormalized.slice(-len).split('').reverse().join('')
  );
}
...
function findLongestWord(sentence) {
  ...
  const maxLen = Math.max(...words.map((s) => s.length));
  return words.find((s) => s.length === maxLen);
}
...
function invertCase(str) {
  return str
    .split('')
    .map((char) => {
      if (char.charCodeAt(0) > 91) return char.toUpperCase();
...
function encodeToRot13(str) {
  ...
  const encryptChar = (c) => {
    ...
    let offset;
    if (code >= 65 && code <= 90) offset = 65;
    else if (code >= 97 && code <= 122) offset = 97;
    else return c;

    const newCode = (code - offset + 13) % 26;
...
function getCardId(value) {
  ...
  return deck.findIndex((cur) => cur === value);
}
